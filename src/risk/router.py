...
async def list_events(
    group_id: UUID = Query(..., description="Group ID to filter events"),
...
async def get_event(
    event_id: UUID,
    group_id: UUID = Query(..., description="Group ID for access control"),
    auth: GroupContext = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
...
async def acknowledge_event(
    ...
    event = await acknowledge_risk_event(db, event_id, group_id, data)
    return risk_event_to_response(event)
...
async def get_config(
    ...
    configs = await get_risk_config(db, group_id)
...
async def update_config(
    ...
    config = await update_risk_config(db, group_id, category, data)
