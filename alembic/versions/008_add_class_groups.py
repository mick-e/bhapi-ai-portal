...
def upgrade() -> None:
    ...
    if not table_exists("class_groups"):
        op.create_table(
            "class_groups",
    ...
    if not index_exists("class_groups", "ix_class_groups_group_id"):
        op.create_index("ix_class_groups_group_id", "class_groups", ["group_id"])

    # 2. class_group_members table
    if not table_exists("class_group_members"):
        op.create_table(
            "class_group_members",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
    ...
    if not index_exists("class_group_members", "ix_class_group_members_class_group_id"):
        op.create_index("ix_class_group_members_class_group_id", "class_group_members", ["class_group_id"])
    if not index_exists("class_group_members", "ix_class_group_members_member_id"):
