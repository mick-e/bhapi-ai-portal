...
def upgrade() -> None:
    ...
    # oauth_clients
    # ------------------------------------------------------------------
    if not table_exists("oauth_clients"):
        op.create_table(
            "oauth_clients",
    ...
    if not index_exists("oauth_clients", "ix_oauth_clients_client_id"):
    ...
    if not index_exists("oauth_tokens", "ix_oauth_tokens_client_id"):
        op.create_index("ix_oauth_tokens_client_id", "oauth_tokens", ["client_id"])
    if not index_exists("oauth_tokens", "ix_oauth_tokens_access_token_hash"):
        op.create_index(
    ...
    # api_key_tiers
    # ------------------------------------------------------------------
    if not table_exists("api_key_tiers"):
        op.create_table(
            "api_key_tiers",
            sa.Column("id", PG_UUID(as_uuid=True), primary_key=True, nullable=False),
    ...
    if not index_exists("api_key_tiers", "ix_api_key_tiers_name"):
        op.create_index("ix_api_key_tiers_name", "api_key_tiers", ["name"], unique=True)

    # ------------------------------------------------------------------
    # platform_webhook_endpoints
    # ------------------------------------------------------------------
    if not table_exists("platform_webhook_endpoints"):
        op.create_table(
            "platform_webhook_endpoints",
            sa.Column("id", PG_UUID(as_uuid=True), primary_key=True, nullable=False),
            sa.Column(
    ...
    if not index_exists("platform_webhook_endpoints", "ix_platform_webhook_endpoints_client_id"):
        op.create_index(
    ...
    # ------------------------------------------------------------------
    if not table_exists("platform_webhook_deliveries"):
        op.create_table(
            "platform_webhook_deliveries",
            sa.Column("id", PG_UUID(as_uuid=True), primary_key=True, nullable=False),
            sa.Column(
    ...
    if not index_exists("platform_webhook_deliveries", "ix_platform_wh_deliveries_endpoint_created"):
        op.create_index(
    ...
    # api_usage_records
    # ------------------------------------------------------------------
    if not table_exists("api_usage_records"):
        op.create_table(
            "api_usage_records",
            sa.Column("id", PG_UUID(as_uuid=True), primary_key=True, nullable=False),
            sa.Column(
    ...
    if not index_exists("api_usage_records", "ix_api_usage_client_date"):
        op.create_index(
